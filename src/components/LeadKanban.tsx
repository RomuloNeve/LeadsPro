...
import { Lead } from "@/hooks/useUserData";
import { LEAD_STATUSES, LeadStatus, getStatusConfig } from "@/components/LeadStatusBadge";
import { LeadScoreBadge } from "@/components/LeadScoreBadge";
import { Badge } from "@/components/ui/badge";
import { Phone, Mail, Instagram, Globe, Linkedin, MessageCircle, GripVertical } from "lucide-react";
...
const LeadKanban = ({ leads, onChangeStatus }: LeadKanbanProps) => {
  ...
  const handleDrop = (e: React.DragEvent, status: string) => {
    e.preventDefault();
    if (draggedLead) {
      onChangeStatus(draggedLead, status as LeadStatus);
    }
    setDraggedLead(null);
      ...
      {LEAD_STATUSES.map((statusConfig) => {
        ...
        return (
          <div
            ...
            className={`flex-shrink-0 w-[260px] rounded-xl border bg-card transition-all ${
